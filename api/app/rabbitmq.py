...
async def publish_task(routing_key: str, payload: dict):
    """Publishes a task to the main tasks exchange."""
    conn = await get_rabbitmq_connection()
    async with conn:
        ch = await conn.channel()
        # This function assumes the exchange is already declared by the app startup logic
        ex = await _compat_get_exchange(ch, TASKS_EXCHANGE)
        if ex is None:
            # Fallback declaration if needed
            ex = await _compat_declare_exchange(ch, TASKS_EXCHANGE, ExchangeType.TOPIC, durable=True)
